...
<body
    class="h-full flex flex-col font-mono antialiased bg-slate-100"
    x-data="threatDashboard({{ $initialEvents->toJson() }})"
>
    ...
    <header class="border-b border-slate-200 bg-white px-6 py-4 flex justify-between items-center shadow-xs">
        ...
        <div class="flex items-center space-x-4 text-xs">
            <div class="bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-md flex items-center space-x-2 shadow-2xs">
                ...
                <span class="text-cyan-700 font-bold uppercase tracking-wider">
                    OPTIONAL
                </span>
            </div>
        </div>
    ...
    <main class="flex-1 p-6 grid grid-cols-1 lg:grid-cols-4 gap-6 overflow-hidden">
        ...
        <section class="lg:col-span-3 flex flex-col gap-6 overflow-hidden">
            ...
            <div class="border border-slate-200 rounded-xl bg-white p-5 relative shadow-xs flex flex-col h-80">
                ...
                <div
                    id="map"
                    class="w-full flex-1 rounded-lg border border-slate-200 bg-slate-50 z-0"
                ></div>
            </div>

            <div class="flex-1 border border-slate-200 rounded-xl bg-white p-5 flex flex-col overflow-hidden shadow-xs">
                <div class="flex justify-between items-center mb-3 pb-2 border-b border-slate-100">
                    <h2 class="text-xs font-bold uppercase tracking-widest text-cyan-700">
                        Live Telemetry Feed
                    </h2>

                    <span
                        class="text-xs text-slate-400"
                        x-text="`Showing latest ${events.length} events`"
                    ></span>
                </div>
                    ...
                    <template x-for="event in events" :key="event.id">
                        ...
                        >
                            <div class="flex items-center space-x-3">
                                ...
                                <span
                                    class="px-2.5 py-1 text-[10px] font-black tracking-wider uppercase rounded-md shadow-2xs"
                                    :class="{
                                ...
                                <div>
                                    <h3
                                        class="text-xs font-bold text-slate-900 group-hover:text-cyan-700 transition"
                                        x-text="event.threat_type"
                                    ></h3>
                            ...
                            <div class="text-right">
                                ...
                                <span
                                    class="px-2 py-0.5 text-[9px] font-bold bg-slate-200 text-slate-700 rounded border border-slate-300 uppercase tracking-widest inline-block mb-1"
                                    x-text="event.location || 'UNKNOWN'"
                                ></span>

                                <span
                                    class="text-[11px] text-slate-400 block"
                                    x-text="formatTime(event.created_at)"
                                ></span>
                            </div>
                        </div>
                    </template>
            ...
            <div class="border border-slate-200 rounded-xl bg-white p-5 shadow-xs space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4 text-center">
                    ...
                    <div class="p-3 bg-slate-50 border border-slate-200 rounded-lg flex flex-col items-center">
                        ...
                        <span class="text-[9px] text-slate-500 font-bold uppercase tracking-widest mb-1">
                            Global Threat
                        </span>
                        ...
                        <div class="relative w-20 h-20 flex items-center justify-center">
                            ...
                            >
                                ...
                                <path
                                    :class="getThreatScore() >= 50 ? 'text-red-500' : 'text-emerald-500'"
                            ...
                            <span
                                class="absolute text-xs font-black text-slate-900"
                                x-text="getThreatScore() + '%'"
                            ></span>
                        </div>
                    </div>
                ...
                <div class="p-4 bg-slate-50 border border-slate-200 rounded-lg">
                    ...
                    <span class="text-[10px] text-slate-500 font-bold uppercase tracking-wider block mb-1">
                        Total Threats Captured
                    </span>
                ...
                <div class="p-4 bg-white border border-slate-200 rounded-lg space-y-2 text-xs shadow-2xs">
                    ...
                    <span class="text-[10px] text-slate-400 uppercase tracking-wider block font-bold mb-2">
                        Severity Breakdown
                    </span>
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span
                            ...
                            x-text="getSeverityCount('critical')"
                        ></span>
                    </div>
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span
                            class="font-bold text-slate-900"
                            x-text="getSeverityCount('medium')"
                        ></span>
                    </div>
    ...
    >
        <div
            ...
            class="bg-white border border-slate-200 rounded-xl p-6 max-w-lg w-full shadow-2xl space-y-4"
        >
            <div class="flex justify-between items-start border-b border-slate-100 pb-3">

                <h3
                    class="text-base font-bold text-slate-900"
                    x-text="selectedEvent?.threat_type"
                ></h3>
            ...
            <div class="space-y-2 text-xs">
                ...
                <p>
                    ...
                    <span
                        class="text-cyan-700 font-bold"
                        x-text="selectedEvent?.source_ip"
                    ></span>
                </p>
                ...
                <p>
                    ...
                    <span
                        ...
                        x-text="selectedEvent?.destination_ip"
                    ></span>
                </p>
                ...
                <div class="mt-4">
                    ...
                    <span class="text-slate-500 block font-bold mb-1">
                        Payload / Telemetry Log:
                    </span>

                    <pre
                        class="bg-slate-900 p-3 rounded-lg border border-slate-800 text-xs text-emerald-400 overflow-x-auto whitespace-pre-wrap font-mono"
                        x-text="selectedEvent?.payload_details || 'SELECT * FROM users WHERE id = 1 OR 1=1;'"
                    ></pre>
                </div>
            </div>
        </div>
        ...
        function threatDashboard(initialEvents) {
            return {
                ...
                init() {
                    ...
                    /*
                     * Plot the threat events already loaded by Laravel.
                     * This makes the attack map useful even when Reverb
                     * is not configured on the public demo.
                     */
                    this.events.forEach((event, index) => {
                        setTimeout(() => {
                            this.plotThreatOnMap(event);
                        }, index * 250);
                    });

                    /*
                     * Reverb is optional.
                     * If configured, future threat events are plotted live.
                     */
                    if (window.Echo) {
                        window.Echo
                            .channel('threat-telemetry')
                ...
                plotThreatOnMap(event) {
                    ...
                    /*
                     * SOC headquarters / destination marker.
                     * Washington D.C. is used as the simulated HQ location.
                     */
                    const dstCoords = [
                        38.9072,
                        -77.0369
                    ...
                    /*
                     * Keep the attack visualization visible long enough
                     * for users and recruiters to actually see it.
                     */
                    setTimeout(() => {
                        if (!this.map) {
                            return;
                ...
                getThreatScore() {
                    ...
                    const criticals =
                        this.getSeverityCount('critical') * 3;

                    const highs =
                        this.getSeverityCount('high') * 2;
